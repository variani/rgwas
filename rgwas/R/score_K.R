...
score_k	<- function( Yb, Yq, G, X=NULL, K, n.folds=10, thr = 0.95, ... ){
	...
	score_matrix	<- matrix( NA, n.folds, 4 )
	cat( 'running fold ' )
	for( kk in 1:n.folds ){
		cat( kk, '...' )
		sub			<- subs[[kk]]
		...
		if( K > 1 ){
			run <- mfmr( trn_Yb, trn_Yq, trn_G, trn_X, K, ... )
			out	<- run$out
			if( !is.null(X) ){
				Xalpha	<- tst_X %*% out$alpha
			} else {
			...
			score_matrix[kk,1]	<- ll_fxn(		tst_Yb, tst_Yq, Xalpha, tst_G, out$beta	, out$Sigma, out$pvec	, Ntest, P+B, K, B )
			score_matrix[kk,2]	<- -sum( out$pvec*log(out$pvec) )
			score_matrix[kk,3]	<- sum(apply(run$pmat, 1, function(x) 
				sum(-x*log(x+1e-10))))			
			score_matrix[kk,4]	<- sum(rowSums(run$pmat > thr)) / nrow(run$pmat)
		} else {
			out	<- mfmr_em_alg_hom( trn_Yb, trn_Yq					, X=cbind( trn_G, trn_X ), ... )
			score_matrix[kk,1]	<- ll_fxn_hom(tst_Yb, tst_Yq, Xalpha=cbind( tst_G, tst_X ) %*% out$alpha, out$Sigma, Ntest, P+B, B )
		}
